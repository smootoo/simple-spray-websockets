    ...
    new OptionalPipelineStage[SslTlsContext] {
      ...
      def applyIfEnabled(context: SslTlsContext, commandPL: CPL, eventPL: EPL): Pipelines =
        new DynamicPipelines {
          ...
          def defaultState(
            remainingOutgoingData: Stream[WriteChunk] = Stream.empty,
            writeWantsAck: Option[WriteChunk] = None,
            closedEvent: Option[Tcp.ConnectionClosed] = None
          ): State = new State {
          ...
          def finishingClose(
            closedEvent: Option[Tcp.ConnectionClosed] = None,
            closeCommand: Tcp.CloseCommand = Tcp.ConfirmedClose
          ): State = new State {
